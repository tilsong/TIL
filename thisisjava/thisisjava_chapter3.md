...
- 다만 곱셈과 나눗셈을 할 때는 연산 후 실수화한다. 따라서 
  ...
  2. 피연산자 하나를 double타입으로 강제 타입변환한 후 산술 연산을 하면 된다.

- 자바는 리터럴 간의 연산은 타입변환 없이 해당 타입으로 계산한다. 따라서 다음과 같은 char 타입은 문제가 없다.
...
이 때 c1은 int 타입으로 변환이 된 후에 1과 연산이 되기 때문이다.
...
- 그러나 실수타입인 0.0 또는 0.0f로 나누면 ArithmeticException이 발생하지 않고, /연산은 Infinity(무한대)값으로, %값은 NaN(Not a Number)을 가지게 된다. 이 값들은 산술연산이 가능하나, 어떤 수와 연산하더라도 Infinity와 NaN이 산출되어 계산이 엉망이 된다.
...
- 부동소수점(실수)을 입력받을 때는 반드시 NaN검사를 해야한다.
...
- (&&, &, |, ||, ~, !) &보다는 &&이, |보다는 ||이 더 효율적으로 동작한다. 왜냐하면 앞의 연산을 끝낸 뒤 해당 결과로 결과 값을 가져올 수 있기 때문이다. 
  ...
  -> if( (x>0) && (y+1) ) 에서 'x-1'이 틀렸을 경우 'y+1'의 연산을 수행하지 않고 바로 false를 제시할 수 있다는 것이다. 
...
- 비트 이동 연산자라는 것이 있다. ~~음.. 솔직히 언제 사용하게 되는 것인지 제일 모르겠는 부분이다.~~
...
- 조건 연산식이라고 할 수 있다. 조건식이 true일 경우 피연산자2, false일 경우 피연산자3을 값으로 가진다. 조건식(피연산자1)  ?   값 또는 연산식(피연산자2)  :  값 또는 연산식(피연산자3)
